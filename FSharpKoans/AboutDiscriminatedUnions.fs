...
module ``08: The Good Kind of Discrimination`` = 
    ...
    let ``02 Creating & pattern-matching a discriminated union`` () = 
        ...
        randomOpinion (BSc (ZooBom, Mathematics)) |> should equal "!!SCIENCE!!"
    ...
    let ``04 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a = Node ("a", Node("a",Node("a",Node("a",Empty,Empty),Empty),Empty) , Node("a",Empty,Empty)) // <-- you may want to spread this over multiple lines and/or let-bindings ...!
